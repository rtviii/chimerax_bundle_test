...
# def print(session, atoms, weighted=False, transformed=True):
...
#     )
def print_profile(session, rcsb_id):
    session.logger.info("rcsb_id: %s" % rcsb_id)



def by_class(session, atoms, color, weighted=False, transformed=True, count=1):
...
def get_ribxz_profile(session, rcsb_id, ignore_cache=True, **kw):
    ...
    url =  "http://127.0.0.1:8000/comp/get_profile/"
